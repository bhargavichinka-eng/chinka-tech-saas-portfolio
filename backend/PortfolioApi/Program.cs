...
var backendEnabled = builder.Configuration.GetValue<bool>("BackendEnabled")
    || builder.Configuration.GetValue<bool>("FeatureManagement:BackendEnabled")
    || (Environment.GetEnvironmentVariable("ENABLE_BACKEND")?.Equals("true", StringComparison.OrdinalIgnoreCase) ?? false);
...
if (backendEnabled && app.Environment.IsDevelopment())
{
    app.UseSwagger();
    app.UseSwaggerUI();
}
